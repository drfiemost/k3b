...
bool K3bLibsndfileDecoder::openFile()
{
    if( !d->isOpen ) {
        ...
        d->sndinfo.format = 0;
        d->sndfile = sf_open (QFile::encodeName(filename()).constData(), SFM_READ, &d->sndinfo);
        if ( !d->sndfile ) {
...
bool K3bLibsndfileDecoderFactory::canDecode( const KUrl& url )
{
    SF_INFO infos;
    infos.format = 0;
    SNDFILE* sndfile = sf_open (QFile::encodeName(url.toLocalFile()).constData(), SFM_READ, &infos);
