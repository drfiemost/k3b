...
class K3bQProcessManager : public QThread
{
    Q_OBJECT
public:
    K3bQProcessManager();
...
private:
    QMutex mutex;
    QMap<int, K3bQProcessInfo *> children;
};

Q_GLOBAL_STATIC(K3bQProcessManager, processManager)

K3bQProcessManager::K3bQProcessManager()
{
#if defined (QPROCESS_DEBUG)
    qDebug() << "K3bQProcessManager::K3bQProcessManager()";
#endif
...
#if defined (QPROCESS_DEBUG)
        qDebug() << "K3bQProcessManager::run() waiting for children to die";
#endif
...
void K3bQProcessManager::catchDeadChildren()
{
    ...
    QMap<int, K3bQProcessInfo *>::Iterator it = children.begin();
    while (it != children.end()) {
        // notify all children that they may have died. they need to run
        // waitpid() in their own thread.
        K3bQProcessInfo *info = it.value();
        qt_native_write(info->deathPipe, "", 1);

#if defined (QPROCESS_DEBUG)
        qDebug() << "K3bQProcessManager::run() sending death notice to" << info->process;
#endif
        ++it;
    }
}
...
void K3bQProcessManager::add(pid_t pid, K3bQProcess *process)
{
#if defined (QPROCESS_DEBUG)
    qDebug() << "K3bQProcessManager::add() adding pid" << pid << "process" << process;
#endif

    // insert a new info structure for this process
    K3bQProcessInfo *info = new K3bQProcessInfo;
...
void K3bQProcessManager::remove(K3bQProcess *process)
{
    QMutexLocker locker(&mutex);

    int serial = process->d_func()->serial;
    K3bQProcessInfo *info = children.value(serial);
    if (!info)
        return;

#if defined (QPROCESS_DEBUG)
    qDebug() << "K3bQProcessManager::remove() removing pid" << info->pid << "process" << info->process;
#endif
...
void K3bQProcessManager::lock()
{
    mutex.lock();
}

void K3bQProcessManager::unlock()
{
    mutex.unlock();
}
...
void K3bQProcessPrivate::destroyPipe(int *pipe)
{
    if (pipe[1] != -1) {
        qt_native_close(pipe[1]);
        pipe[1] = -1;
...
//        if (threadData->eventDispatcher) {
            ...
            } else if ( &channel == &stderrChannel || !(processFlags&K3bQProcess::RawStdout) ){
...
#if defined (QPROCESS_DEBUG)
    qDebug("K3bQProcessPrivate::startProcess()");
#endif
...
//    }
    ...
    if (childPid < 0) {
        ...
        processError = ::QProcess::FailedToStart;
        q->setErrorString(K3bQProcess::tr("Resource error (fork failure): %1").arg(qt_error_string(lastForkErrno)));
        emit q->error(processError);
        cleanup();
        return;
    }
    ...
    if (stdinChannel.pipe[1] != -1 && !(processFlags&K3bQProcess::RawStdin))
    ...
    if (stdoutChannel.pipe[0] != -1 && !(processFlags&K3bQProcess::RawStdout))
...
void K3bQProcessPrivate::execChild(const char *workingDir, char **path, char **argv, char **envp)
{
    ...
    if (environment.isEmpty()) {
        ...
        if (path) {
            ...
            while (*arg) {
                argv[0] = *arg;
#if defined (QPROCESS_DEBUG)
                fprintf(stderr, "K3bQProcessPrivate::execChild() searching / starting %s\n", argv[0]);
#endif
...
#if defined (QPROCESS_DEBUG)
            fprintf(stderr, "K3bQProcessPrivate::execChild() starting %s\n", argv[0]);
#endif
            qt_native_execve(argv[0], argv, envp);
        }
    }
    ...
    // notify failure
#if defined (QPROCESS_DEBUG)
    fprintf(stderr, "K3bQProcessPrivate::execChild() failed, notifying parent process\n");
#endif
...
#if defined (QPROCESS_DEBUG)
    qDebug("K3bQProcessPrivate::processStarted() == %s", i <= 0 ? "true" : "false");
#endif
    return i <= 0;
}
...
#if defined QPROCESS_DEBUG
    qDebug("K3bQProcessPrivate::readFromStdout(%p \"%s\", %lld) == %lld",
...
#if defined QPROCESS_DEBUG
    qDebug("K3bQProcessPrivate::readFromStderr(%p \"%s\", %lld) == %lld",
           data, qt_prettyDebug(data, bytesRead, 16).constData(), maxlen, bytesRead);
#endif
    return bytesRead;
}
...
#if defined QPROCESS_DEBUG
    qDebug("K3bQProcessPrivate::writeToStdin(%p \"%s\", %lld) == %lld",
...
{
#if defined (QPROCESS_DEBUG)
    qDebug("K3bQProcessPrivate::killProcess()");
#endif
    if (pid)
...
#if defined (QPROCESS_DEBUG)
    qDebug("K3bQProcessPrivate::waitForStarted(%d) waiting for child to start (fd = %d)", msecs,
	   childStartedPipe[0]);
#endif
...
#if defined (QPROCESS_DEBUG)
        qDebug("K3bQProcessPrivate::waitForStarted(%d) == false (timed out)", msecs);
#endif
        return false;
    }
...
#if defined (QPROCESS_DEBUG)
    qDebug("K3bQProcessPrivate::waitForStarted() == %s", startedEmitted ? "true" : "false");
#endif
...
bool K3bQProcessPrivate::waitForBytesWritten(int msecs)
{
    Q_Q(K3bQProcess);
#if defined (QPROCESS_DEBUG)
    qDebug("K3bQProcessPrivate::waitForBytesWritten(%d)", msecs);
#endif
...
bool K3bQProcessPrivate::waitForDeadChild()
{
    Q_Q(K3bQProcess);
...
#if defined QPROCESS_DEBUG
    qDebug() << "K3bQProcessPrivate::waitForDeadChild() not dead!";
#endif
    return false;
}
...
/*! \internal
 */
bool K3bQProcessPrivate::startDetached(const QString &program, const QStringList &arguments, const QString &workingDirectory, qint64 *pid)
{
    processManager()->start();
...
void K3bQProcessPrivate::initializeProcessManager()
{
    (void) processManager();
}
